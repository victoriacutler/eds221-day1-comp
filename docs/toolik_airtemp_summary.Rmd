...
knitr::opts_chunk$set(echo = TRUE, message = FALSE, warning = FALSE) ##global options that code units will show up but messages and warning will not show up in our knitted version
library(tidyverse)
library(here)
library(janitor)
library(skimr)
...
```{r}
toolik_weather <- read_csv(here::here("data", "data-raw", "toolik_weather.csv")) ##there's also "read.csv" but "read_csv" is better at making class decisions
```
...
toolik_temp_plot <- ggplot(data = toolik_weather, aes(x = as.factor(month), y = daily_air_temp_mean_c)) +
  ...
  theme_minimal() +
  labs(title = "Toolik Station daily air temperature (C) vs Month",
       x = "Month",
